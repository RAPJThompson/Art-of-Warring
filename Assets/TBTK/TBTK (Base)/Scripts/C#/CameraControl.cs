...
public class CameraControl : MonoBehaviour {
	...
	public bool trackUnit=false;
	private UnitTB lastSelectedUnit;
	private bool tracking=true;
	
	// Update is called once per frame
	void Update () {
		if(trackUnit && UnitControl.selectedUnit!=null){
			if(lastSelectedUnit!=UnitControl.selectedUnit){
				lastSelectedUnit=UnitControl.selectedUnit;
				tracking=true;
			}
			
			if(tracking){
				float dist=Vector3.Distance(thisT.position, lastSelectedUnit.thisT.position)*.025f;
				dist=Mathf.Max(1, dist);
				thisT.position=Vector3.Lerp(thisT.position, lastSelectedUnit.thisT.position, Time.deltaTime*5*dist);
			}
		}
		...
		if(enableKeyPanning){
			...
			if(Input.GetButton("Vertical")) { tracking=false;
		...
		if(enableMousePanning){
			...
			if(mousePos.x<=0){ tracking=false;
				thisT.Translate(dirHor * panSpeed * deltaT * -3);
			}
			else if(mousePos.x<=mousePanningZoneWidth){ tracking=false;
				thisT.Translate(dirHor * panSpeed * deltaT * -1);
			}
			else if(mousePos.x>=Screen.width){ tracking=false;
				thisT.Translate(dirHor * panSpeed * deltaT * 3);
			}
			else if(mousePos.x>Screen.width-mousePanningZoneWidth){ tracking=false;
				thisT.Translate(dirHor * panSpeed * deltaT * 1);
			}
			
			Vector3 dirVer=transform.InverseTransformDirection(direction*Vector3.forward);
			if(mousePos.y<=0){ tracking=false;
				thisT.Translate(dirVer * panSpeed * deltaT * -3);
			}
			else if(mousePos.y<=mousePanningZoneWidth){ tracking=false;
				thisT.Translate(dirVer * panSpeed * deltaT * -1);
			}
			else if(mousePos.y>=Screen.height){ tracking=false;
				thisT.Translate(dirVer * panSpeed * deltaT * 3);
			}
			else if(mousePos.y>Screen.height-mousePanningZoneWidth){ tracking=false;
				thisT.Translate(dirVer * panSpeed * deltaT * 1);
			}
		}
