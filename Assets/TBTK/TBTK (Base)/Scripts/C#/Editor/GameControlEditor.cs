...
//***************************************************************************************************************************//
		string heightText="check to use enable vertical axis in the game\n";
		heightText+="Grid will auto-adjust to match the terrain height\n";
		heightText+="Unit on higher ground will gain hit-bonus when attacking unit on lower ground\n";
		heightText+="Unit on lower ground will get hit-penalty when attacking unit on higher ground\n";
		heightText+="\nPlease note that this is a relatively new Feature and therefore may contain bugs";
		
		cont=new GUIContent("Enable Vertical Component:", heightText);
		EditorGUILayout.BeginHorizontal("");
		EditorGUILayout.LabelField(cont, GUILayout.MinWidth(180));
		gc.enableHeight=EditorGUILayout.Toggle(gc.enableHeight, GUILayout.ExpandWidth(true));
		EditorGUILayout.EndHorizontal();
		
		if(gc.enableHeight){
			cont=new GUIContent("- Minimum height:", "minimum height difference (in height level, as determined based on heightUnit) required before the hit bonus/penalty is applied ");
			gc.minHeightDifference=EditorGUILayout.IntField(cont, gc.minHeightDifference);
			
			cont=new GUIContent("- highToLow Bonus:", "hit bonus gained when a unit is attacking from high ground\ntakes value from 0-1, 1 being 100%");
			gc.highToLowHitBonus=EditorGUILayout.FloatField(cont, gc.highToLowHitBonus);
			
			cont=new GUIContent("- lowToHigh Penalty:", "hit penalty gained when a unit is attacking from low ground\ntakes value from 0-1, 1 being 100%");
			gc.lowToHighHitPenalty=EditorGUILayout.FloatField(cont, gc.lowToHighHitPenalty);
			
			cont=new GUIContent("- Height Unit:", "base height difference unit used to determine height level for each tile\nwhen set to 1, any tile with y-axis pos between 0-1 is considered at level-0, any tile with y-axis pos between 1-2 is considered at level-1 and so on");
			gc.heightUnit=EditorGUILayout.FloatField(cont, gc.heightUnit);
			
			cont=new GUIContent("- Move Cost:", "movement range required to traverse each height level in an adjacent tile\nif a neighbouring tile is 2 level higher/lower, then it will cost a unit 2 movement range to move to that tile.");
			gc.moveCostPerHeight=EditorGUILayout.IntField(cont, gc.moveCostPerHeight);
		}
