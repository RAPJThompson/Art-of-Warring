...
public class GridManagerEditor : Editor {
	...
	public override void OnInspectorGUI(){
		...
		EditorGUILayout.Space();
			...
			cont=new GUIContent("Height Offset From Terrain:", "the y-axis spacing between the tile and the terrain");
			gm.terrainHeightOffset=EditorGUILayout.FloatField(cont, gm.terrainHeightOffset);
			cont=new GUIContent("Map Grid to Terrain", "Re-adjust the height of the grid to fit the terrain");
			if(GUILayout.Button(cont)) MapHeightToTerrain();
		EditorGUILayout.Space();
	...
	public void MapHeightToTerrain(){
		...
		foreach(Tile tile in allTilesInScene){
			tile.AdjustHeight(gm.baseHeight, gm.terrainHeightOffset);
		}
	}
	...
	public static List<Tile> GenerateSquareGrid(GridManager gm){
		...
		int m=0, n=0, totalCount=(int)gm.width*(int)gm.length;
		for(int i=0; i<totalCount; i++){
			Vector3 pos=new Vector3(m*gridSizeFactor, gm.baseHeight, n*gridSizeFactor);
			...
			GameObject obj=(GameObject)Instantiate(gm.squareTilePrefab);
			...
			obj.transform.localRotation=Quaternion.Euler(-90, 0, 0);
				...
				counter+=1;
			#endif
			obj.transform.localScale*=gm.gridSize;
	...
	void GenerateGrid(){
		...
		*/
		...
		//if(gm.mapHeightToTerrain){
		//	for(int i=0; i<tileList.Count; i++){
		//		tileList[i].AdjustHeight(gm.baseHeight);
		//	}
		//}
