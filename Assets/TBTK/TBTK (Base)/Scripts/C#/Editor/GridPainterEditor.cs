				...
				*/
				...
				EditorGUILayout.BeginHorizontal();
					for(int i=0; i<collectibleList.Count; i++){
						...
						if(collectibleID==i && Event.current.type==EventType.repaint){
							selectedItemRect=GUILayoutUtility.GetLastRect();
							Repaint();
						}
					}
					...
					foreach(Tile tile in allTiles){
						...
						foreach(Wall wall in tile.walls){
							if(wall.tile1!=null && wall.tile2!=null){
								...
								Vector3 v=new Vector3(Mathf.Cos(wall.angle*Mathf.Deg2Rad), 0, Mathf.Sin(wall.angle*Mathf.Deg2Rad));
								Handles.DrawLine(tile.pos, tile.pos+v*gridManager.gridSize);
							}
						}
					}
				}
