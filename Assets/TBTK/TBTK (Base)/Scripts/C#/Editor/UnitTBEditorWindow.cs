...
public class UnitTBEditorWindow : EditorWindow {
	...
	float UnitTBAudioConfigurator(int ID, float startX, float startY){
		...
		if(showAudio){
			...
			cont=new GUIContent("   MissedSoundRange:", "Audio clip to play when the unit is hit by range attack");
			audio.missedSoundRange=(AudioClip)EditorGUI.ObjectField(new Rect(startX, startY+=spaceY, 300, 17), cont, audio.missedSoundRange, typeof(AudioClip), false);
			
			cont=new GUIContent("   MissedSoundMelee:", "Audio clip to play when the unit is hit by melee attack");
			audio.missedSoundMelee=(AudioClip)EditorGUI.ObjectField(new Rect(startX, startY+=spaceY, 300, 17), cont, audio.missedSoundMelee, typeof(AudioClip), false);
