...
public class GridManager : MonoBehaviour{
	...
	void Awake(){
		...
		//neighbour is no longer serialized, it's setup in runtime
		//this snippet to setup the neighbour, all tile needs to be on a leveled height to setup neighbour
		for(int i=0; i<allTiles.Count; i++){
			allTiles[i].transform.position=new Vector3(allTiles[i].pos.x, baseHeight, allTiles[i].pos.z);
		}
		for(int i=0; i<allTiles.Count; i++){
			allTiles[i].SetupNeighbour();
		}
		for(int i=0; i<allTiles.Count; i++){
			allTiles[i].transform.position=new Vector3(allTiles[i].pos.x, allTiles[i].pos.y, allTiles[i].pos.z);
		}
		
		//setup walls after neighbour is setup
		for(int i=0; i<allTiles.Count; i++){
			allTiles[i].SetupWall();
		}
	}
	...
	void GenerateSquareGrid(){
		...
		//~ int m=0, n=0, totalCount=(int)width*(int)length;
		//~ for(int i=0; i<totalCount; i++){
			//~ Vector3 pos=new Vector3(m*gridSizeFactor, baseHeight, n*gridSizeFactor);
		
			//~ if(pos.z>highestY) highestY=pos.z;				if(pos.z<lowestY) lowestY=pos.z;
			//~ if(pos.x>highestX) highestX=pos.x;				if(pos.x<lowestX) lowestX=pos.x;
		...
		Debug.Log("Generate grid, time:"+timeSpan.TotalMilliseconds);
	...
	public void GenerateGrid(bool generateNew){
		...
		*/
		...
		if(generateNew){
			timeE=System.DateTime.Now;
			timeSpan=timeE-timeS;
			Debug.Log("Everything else, time:"+timeSpan.TotalMilliseconds);
		}
	...
	public static void Select(Tile ht){
		...
		if(ht.unit.factionID!=UnitControl.selectedUnit.factionID) return;
...
//code for fog of war calculation
	...
	public static bool IsInLOS(Vector3 pos1, Vector3 pos2, bool debugging){
		if(GameControlTB.EnableHeightTB()){
			float hUnit=GameControlTB.GetHeightUnit();
			pos1.y+=hUnit*0.75f;
			pos2.y+=hUnit*0.75f;
		}
...
//utility function with regards to the grid
	...
	*/
	...
	//modified to take height component into consideration
	//call from GetWalkableTilesWithinRangeH(), without H
	public static List<Tile> GetWalkableTilesWithinRangeH(Tile rootHT, int range){
		...
		if(range==1){
			...
			for(int i=0; i<tileNeighbours.Count; i++){
				if(tileNeighbours[i].walkable && tileNeighbours[i].unit!=null){
					if(CheckTileMovablilityToHeight(rootHT, tileNeighbours[i], range)){
						list.Add(tileNeighbours[i]);
					}
				}
			}
			for(int i=0; i<list.Count; i++) list[i].rangeRequired=0;
			return list;
		}
		...
		foreach(Tile neighbour in tileNeighbours){
			if(neighbour.walkable && neighbour.unit==null){
				if(!newOpenList.Contains(neighbour)){
					if(CheckTileMovablilityToHeight(rootHT, neighbour, range)){
						newOpenList.Add(neighbour);
					}
				}
			}
		...
		for(int i=0; i<range; i++){
			...
			foreach(Tile ht in openList){
				foreach(Tile neighbour in ht.GetNeighbours()){
					if(neighbour.walkable && neighbour.unit==null){
						if(!newOpenList.Contains(neighbour) && !closeList.Contains(neighbour)&& !openList.Contains(neighbour)){	//this line must go before CheckTile of the rangeRaquired will be stacked
							if(CheckTileMovablilityToHeight(ht, neighbour, range)){
								newOpenList.Add(neighbour);
							}
						}
						//newOpenList.Add(neighbour);
		...
		for(int i=0; i<closeList.Count; i++) closeList[i].rangeRequired=0;
		for(int i=0; i<openList.Count; i++) openList[i].rangeRequired=0;
		for(int i=0; i<newOpenList.Count; i++) newOpenList[i].rangeRequired=0;
	...
	public static bool CheckTileMovablilityToHeight(Tile srcTile, Tile tgtTile, int range){
		...
		if(srcTile.heightLevel!=tgtTile.heightLevel){
			int currentCost=1+Mathf.Abs(srcTile.heightLevel-tgtTile.heightLevel)*moveCostH;
			tgtTile.rangeRequired=currentCost+srcTile.rangeRequired;
	...
	//default code, no longer in use with height component added?
	public static List<Tile> GetWalkableTilesWithinRange(Tile rootHT, int range){
		if(GameControlTB.EnableHeightTB()){
			return GetWalkableTilesWithinRangeH(rootHT, range);
		}
	...
	public static float GetBaseHeight(){
		return instance.baseHeight;
	}
	public static float GetTerrainHeightOffset(){
		return instance.terrainHeightOffset;
	}
