...
public class Wall{
	...
	public bool Contains(int ang){
		if(Mathf.Abs(ang-angle)<2) return true;
		return false;
	}
}
...
public class Tile : MonoBehaviour {
	...
	//set height to terrain, called in Start()
	public void AdjustHeight(){ AdjustHeight(GridManager.GetBaseHeight(), GridManager.GetTerrainHeightOffset()); }
	public void AdjustHeight(float baseHeight, float heightOffset){
		if(obstacle!=null) obstacle.transform.parent=null;
		
		Vector3 castPos=new Vector3(transform.position.x, baseHeight+5000, transform.position.z);
		
		LayerMask mask=1<<10 | 1<<9;
		//LayerMask mask=1<<9;
		//LayerMask mask=1<<10;
		RaycastHit hit;
        if (Physics.Raycast(castPos, -Vector3.up, out hit, Mathf.Infinity, mask)){
            transform.position=new Vector3(castPos.x, hit.point.y+heightOffset, castPos.z);
		}
		else{
			transform.position=new Vector3(castPos.x, baseHeight, castPos.z);
		}
		
		pos=transform.position;
		
		if(unit!=null) unit.transform.position=pos;
		if(collectible!=null) collectible.transform.position=pos;
		//if(obstacle!=null) obstacle.transform.position=pos;
		//for(int i=0; i<walls.Count; i++){
		//	if(walls[i].wallObj!=null){
		//		Vector3 poss=new Vector3(walls[i].wallObj.transform.position.x, pos.y, walls[i].wallObj.transform.position.z);
		//		walls[i].wallObj.transform.position=poss;
		//	}
		//}
		
		if(obstacle!=null) obstacle.transform.parent=transform;
	}
	
	//to setup adjacent tile as neighbour durin the start of runtime, called bu GridManager
	public void SetupNeighbour(){
		float tileSize=GridManager.GetTileSize();
	
		Vector3 poss=new Vector3(transform.position.x, GridManager.GetBaseHeight(), transform.position.z);
		
		Collider[] cols=Physics.OverlapSphere(poss, tileSize*0.6f);
		List<Tile> neighbourList=new List<Tile>();
		
		foreach(Collider col in cols){
			Tile neighbourTile=col.gameObject.GetComponent<Tile>();
			if(neighbourTile!=null && neighbourTile!=this){
	...
	[HideInInspector] public int rangeRequired;	//the range-cost required to move to this tile, for grid with height variance
	...
	public virtual void Start () {
		...
		if(GameControlTB.EnableHeightTB()){
			...
			float heightUnit=GameControlTB.GetHeightUnit();
			float baseHeight=GridManager.GetBaseHeight();
			float dif=Mathf.Abs(pos.y-baseHeight)/heightUnit;
			if(pos.y>baseHeight) dif=Mathf.Floor(dif);
			else if(pos.y<baseHeight) dif=-Mathf.Ceil(dif);
			heightLevel=(int)dif;
		}
	...
	//to setup wall in start of runtime, called by GridManager
	public void SetupWall(){
		for(int i=0; i<walls.Count; i++){
			if(walls[i].angle==360) walls[i].angle=0;
			
			for(int n=0; n<neighbours.Count; n++){
				Vector2 dirV=new Vector2(neighbours[n].pos.x-pos.x, neighbours[n].pos.z-pos.z);
				float angleW=Utility.VectorToAngle(dirV);
				
				if((int)angleW==(int)walls[i].angle){
					Tile nTile=neighbours[n];
					
					AddWall(angleW);
					
					float angle=angleW>180 ? angleW-180 : angleW+180;
					nTile.AddWall(angle);
	...
	public Tile AddWall(float angle, Obstacle wallPrefab){
		...
		if(type==_TileType.Hex){
			...
			for(int i=0; i<6; i++){
				if(angle>i*60+30 && angle<=(i+1)*60+30){
					angle=(i+1)*60;
				}
			}
		}
			...
			for(int i=0; i<4; i++){
				if(angle>i*90+45 && angle<=(i+1)*90+45){
					angle=(i+1)*90;
				}
			}
		}
		...
		for(int i=0; i<walls.Count; i++){
			if(walls[i].Contains((int)angle)){
				...
				if(tile!=null){
					tile.walls.Add(new Wall(wall.wallObj, tile, this, angle+180));
		...
		if(wallPrefab!=null){
			...
			float rotOffset=90;
			wallT.rotation=Quaternion.Euler(0, -(angle+rotOffset), 0);
			
			wallT.parent=transform;
		}
		
		walls.Add(new Wall(wallT, tile, this, angle));
		if(tile!=null){
			tile.walls.Add(new Wall(wallT, tile, this, angle+180));
			RemoveNeighbour(tile);
		}
...
//cover system related code section	
	...
	void InitCover(){
		StartCoroutine(_InitCover());
	}
	IEnumerator _InitCover(){
		yield return null;
	...
	void InitCoverHex(){
		...
		foreach(Tile tile in adjacentTile){
			...
			Vector3 direction=tile.pos-pos;	direction.y=0;
			...
			direction=tile.pos-pos; direction.y=Mathf.Max(0, direction.y);
