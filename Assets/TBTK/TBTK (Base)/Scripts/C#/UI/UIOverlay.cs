...
public class UIOverlay : MonoBehaviour {
	...
	void Update () {
		//Debug.Log(effectOverlayList.Count);
	}
	...
	void DrawHoverInfo(){
		...
		if(tileHovered.attackableToSelected){
			...
			dmgMin=Mathf.Max(0, dmgMin-dmgReduc);
			dmgMax=Mathf.Max(0, dmgMax-dmgReduc);
			...
			if(GameControlTB.MovementAPCostRule()==_MovementAPCostRule.None) return;
