...
public class AttackInstance{
	...
	public void Process(UnitTB unit){
		...
		if(GameControlTB.EnableHeightTB()){
			int srcHeight=srcUnit.occupiedTile.heightLevel;
			int targetHeight=targetTile.heightLevel;
			int dif=srcHeight-targetHeight;
			if(dif>0) heightModifier=GameControlTB.GetH2LBonus();
			else if(dif<0) heightModifier=-GameControlTB.GetL2HPenalty();
			
			//Debug.Log(srcHeight+"   "+targetHeight);
		}
		
		//if(heightModifier>0){
		//	new EffectOverlay(srcUnit.occupiedTile.pos, "height bonus", Color.green);
		//}
		//else if(heightModifier<0){
		//	new EffectOverlay(srcUnit.occupiedTile.pos, "height penalty", Color.red);
		//}
		
		float attack=0;
		int damageType=0;
		if(type==_AttackType.Range_Normal || type==_AttackType.Range_Counter){
			attack=srcUnit.GetRangeAttackRaw();
			damageType=srcUnit.damageTypeRange;
		}
		else if(type==_AttackType.Melee_Normal || type==_AttackType.Melee_Counter){
			attack=srcUnit.GetMeleeAttackRaw();
		...
		//~ float missChance=Mathf.Clamp((1-attack)+targetUnit.GetDefend()+coverDefBonus, 0f, 1f);
		//float missChance=Mathf.Clamp(targetUnit.GetDefend()+coverDefBonus-attack-heightModifier, 0f, 1f);
		//Debug.Log("miss chance: "+missChance+"   "+targetUnit.GetDefend()+"    "+coverDefBonus+"   "+attack+"   "+heightModifier);
		float hitChance=Mathf.Clamp(attack+heightModifier-targetUnit.GetDefend()-coverDefBonus, 0, 1f);
		...
		if(Random.Range(0f, 1f)<=critChance+exposedCritBonus){
			...
			dmg=(int)Mathf.Max(0, dmg-targetUnit.GetDmgReduc());
			critical=true;
...
public class UnitTB : MonoBehaviour {
	...
	*/
	...
	IEnumerator MoveRoutine(Tile origin, Tile target){
		...
		thisT.rotation=Quaternion.Euler(0, thisT.rotation.eulerAngles.y, 0);//occupiedTile.rotation*Quaternion.Euler(90, 0, 0);
	...
	public bool Attack(List<Tile> targetTileList, UnitAbility ability){
		...
		if(attackRule==_AttackAPCostRule.PerAttack){
			if(AP<APCostAttack){
				Debug.Log("Insufficient AP");
				GameControlTB.DisplayMessage("Insufficient AP");
				return false;
			}
			AP-=APCostAttack;
		}
				...
				foreach(Tile tile in targetTileList){
					for(int i=0; i<shootPoints.Count; i++){
						...
						if(i==shootPoints.Count-1){
							...
							shootObj.SetAbilityTargetTile(abilityTargetTile);	//for chain ability only
						}
	...
	//called by AI to end the unit move
	public void CompleteAllAction(float delay){
		StartCoroutine(ActionComplete(delay));
	}
	...
	public AttackInstance ApplyHitEffect(AttackInstance attInstance){
		...
		if(attInstance.missed){
			if(attInstance.type==_AttackType.Range_Normal || attInstance.type==_AttackType.Range_Counter)
				audioTB.PlayMissedRange();
			if(attInstance.type==_AttackType.Melee_Normal || attInstance.type==_AttackType.Melee_Counter)
				audioTB.PlayMissedMelee();
		}
		else{
			audioTB.PlayHit();
			animationTB.PlayHit();
	...
	IEnumerator _RotateToUnit(UnitTB unit){
		Vector3 pos1=occupiedTile.pos;
		Vector3 pos2=unit.occupiedTile.pos; pos2.y=pos1.y;
		Quaternion wantedRot=Quaternion.LookRotation(pos2-pos1);
		while(true){
			thisT.rotation=Quaternion.Slerp(thisT.rotation, wantedRot, Time.deltaTime*rotateSpeed);
			if(Quaternion.Angle(thisT.rotation, wantedRot)<1){
				thisT.rotation=wantedRot;
...
//code related to activate unit ability
	...
	public void ApplyAbility(UnitAbility ability){
		if(ability.effectType==_EffectType.Buff || ability.effectType==_EffectType.Debuff){
			foreach(EffectAttr effectAttr in ability.effectAttrs) ApplyAbilityEffectBuff(effectAttr);
			...
			foreach(EffectAttr effectAttr in ability.effectAttrs) ApplyAbilityEffectDirect(effectAttr);
		}
	...
	public void ApplyCollectibleEffect(Effect effect){
		foreach(EffectAttr effectAttr in effect.effectAttrs){
			ApplyAbilityEffectBuff(effectAttr);
		}
...
//*****************************************************************************************************************************
		...
		for(int i=0; i<activeUnitAbilityEffectList.Count; i++){
			...
			if(ability.countTillNextTurn<=0){
				...
				if(ability.effectAttrs.Count==0){
					...
					//Debug.Log(ability.name+", effects remain: "+ability.effectAttrs.Count);
				}
		...
		for(int i=0; i<activeCollectibleAbilityEffectList.Count; i++){
			...
			if(effect.countTillNextTurn<=0){
				...
				if(effect.effectAttrs.Count==0){
					activeCollectibleAbilityEffectList.RemoveAt(i);
					i-=1;
					if(onEffectExpiredE!=null) onEffectExpiredE(this);
				}
	...
	bool CalculateAbilityEffect(EffectAttr effect){
		...
		if(effect.duration<=0){
			if(effect.type==_EffectAttrType.HPDamage){
				...
				if(this==UnitControl.selectedUnit){
					GridManager.ClearWalkableList();
					GridManager.ResetWalkableList(occupiedTile);
				}
			}
				...
				if(this==UnitControl.selectedUnit){
					GridManager.ClearHostileList();
					GridManager.ResetHostileList(occupiedTile);
				}
			}
			...
			else if(effect.type==_EffectAttrType.DamageReduction){
				damageReduc-=(int)effect.value;
			}
	...
	//for buff ability
	void ApplyAbilityEffectBuff(EffectAttr effect){
		if(effect.type==_EffectAttrType.HPDamage){
			...
			ApplyAbilityEffectStat(effect);
		}
		
	}
	
	//function call to apply effectAttr, apply the effect value and edit the modifier (not subject to effectAttr duration)
	//for direct ability
	void ApplyAbilityEffectDirect(EffectAttr effect){
		if(effect.type==_EffectAttrType.HPDamage){
			//first round, any dot after the first round is calculated and applied in CalculateHPDamagePerTurn()
			float val=Random.Range(effect.value, effect.valueAlt);
			float modifier=DamageTable.GetModifier(armorType, effect.damageType);
	...
	//function call to apply effectAttr, apply the effect value and edit the modifier(subject to effectAttr duration)
	void ApplyAbilityEffectStat(EffectAttr effect){
		if(effect.type==_EffectAttrType.Damage){
			rangeDamageMin+=(int)effect.value;
			rangeDamageMax+=(int)effect.value;
			meleeDamageMin+=(int)effect.value;
			meleeDamageMax+=(int)effect.value;
...
//public function to get various member of the instance
	...
	public float GetUnitRangeAttack(){ return attRange; }
	//~ public float GetUnitMeleeAttack(){ return Mathf.Clamp(attMelee, 0f, 1f); }
	public float GetUnitMeleeAttack(){ return attMelee; }
	...
	public int GetUnitDmgReduc(){ return damageReduc; }//Mathf.Clamp(damageReduc, 0f, 1f); }
	...
	public int GetPerkDmgReduc(){		return bonusDmgReduc;		}
	...
	public int GetTileDmgReduc(){		return 0;		}
	...
	public float GetRangeAttackRaw(){ return GetUnitRangeAttack()+GetPerkRangeAttack()+GetTileRangeAttack(); }
	public float GetMeleeAttackRaw(){ return GetUnitMeleeAttack()+GetPerkMeleeAttack()+GetTileMeleeAttack(); }
	
	public float GetRangeAttack(){ return Mathf.Clamp(GetUnitRangeAttack()+GetPerkRangeAttack()+GetTileRangeAttack(), 0f, 1f); }
	//~ public float GetRangeAttack(){ return GetUnitRangeAttack()+GetPerkRangeAttack()+GetTileRangeAttack(); }
	public float GetMeleeAttack(){ return Mathf.Clamp(GetUnitMeleeAttack()+GetPerkMeleeAttack()+GetTileMeleeAttack(), 0f, 1f); }
	//~ public float GetMeleeAttack(){ return GetUnitMeleeAttack()+GetPerkMeleeAttack()+GetTileMeleeAttack(); }
	...
	public int GetDmgReduc(){ 
		return GetUnitDmgReduc()+GetPerkDmgReduc()+GetTileDmgReduc();
		//return Mathf.Clamp(GetUnitDmgReduc()+GetPerkDmgReduc()+GetTileDmgReduc(), 0f, 1f); }
	}
	...
	public float GetTotalHitChanceMelee(UnitTB target){
		return Mathf.Clamp(GetMeleeAttackRaw()-target.GetDefend(), 0, 1);
	}
	public float GetTotalHitChanceRange(UnitTB target){
		...
		if(GameControlTB.EnableHeightTB()){
			int srcHeight=occupiedTile.heightLevel;
			int targetHeight=target.occupiedTile.heightLevel;
			int dif=srcHeight-targetHeight;
			if(dif>0) heightBonus=GameControlTB.GetH2LBonus();
			else if(dif<0) heightBonus=-GameControlTB.GetL2HPenalty();
		}
		return Mathf.Clamp(GetRangeAttackRaw()+heightBonus-(target.GetDefend()+coverBonus), 0, 1);
	}
...
//move attack, obsolete code
	...
	public bool MoveAttack(Tile destinationTile){
		return MoveAttack(destinationTile, false);
	}
	public bool MoveAttack(Tile destinationTile, bool attackOnly){
		if(stun>0) return false;
		
		//if movement is required but disabled
		if(!attackOnly && occupiedTile!=destinationTile && movementDisabled>0) return false;
	...
	IEnumerator MoveAttackRoutine(Tile origin, Tile target, int range){
		if(origin!=target)	{
			...
			while(path.Count>1){
				...
				thisT.rotation=Quaternion.Euler(0, thisT.rotation.eulerAngles.y, 0);
